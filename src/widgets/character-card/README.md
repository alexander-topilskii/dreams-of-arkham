...
## Purpose
CharacterCard показывает текущий статус исследователя (портрет, описание, здоровье, рассудок, действия и активные эффекты) и размещается в верхней левой панели интерфейса игры Dreams of Arkham.

## Visual Overview
```
┌──────────────────────────────────────────────┐
│┌───────┐┌───────────────────────────────────┐│
││ IMG   ││Агнес Бэйкер          Таинственная ││
││       ││официантка                           │
│└───────┘│Бывшая официантка...               ││
├────────┴────────────────────────────────────┤
│Здоровье 6 / 7 [██████████░░]  Рассудок 5 / 8│
│Действия 3                                    │
├──────────────────────────────────────────────┤
│Активные эффекты                              │
│• Защитный оберег — предотвращает 1 ужас     │
│• Элдрический резонанс — +1 действие...      │
└──────────────────────────────────────────────┘
```
...
- Эффекты отображаются в виде списка карточек, пустое состояние показывает курсивную подпись.
...
const slot = document.getElementById("left-top");
