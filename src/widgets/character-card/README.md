...
- При уменьшении здоровья карточка здоровья на секунду окрашивается в красный оттенок, резко трясётся, выбрасывает красные частички и показывает всплывающий текст `-X`, который падает вниз и затухает.
- При восстановлении здоровья блок мягко вибрирует, зеленеет на секунду и отображает всплывающий текст `+X`, который плавно улетает вверх.
- Враждебные эффекты (враги, сводка «Сражается с …») автоматически выводятся в начале списка, подсвечиваются мягким красным градиентом и при появлении слегка вибрируют.
