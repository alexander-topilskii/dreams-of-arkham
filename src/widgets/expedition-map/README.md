...
Карта экспедиции — виджет для отображения сети территорий в виде карточек.
Каждая территория имеет оборотную и лицевую стороны, связи с другими территориями и поддерживает интерактивность.

## Как выглядит и ведёт себя
- Территории отображаются прямоугольными карточками с мягкими скруглениями, изначально повернутыми задней стороной.
- Лицевая сторона открывается автоматически, когда движок сообщает о посещении территории: показываются имя, описание и фон с иллюстрацией.
...
- Все жетоны персонажей оформлены единым стилем и могут отображать переданный портрет.
...
- `map.revealTerritory(territoryId)` — открывает лицевую сторону указанной территории (обычно вызывается адаптером игрового движка при посещении).
- `map.placeCharacter(character, territoryId)` — добавляет или перемещает жетон персонажа к указанной территории. Если цвет не задан, карта подберёт его автоматически. Параметр `character.image` позволяет показать портрет на жетоне.
