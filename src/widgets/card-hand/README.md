...
└───────────────────────────────────────┘
```
- Контейнер: полупрозрачная панель с мягким свечением, компактными отступами и радиусом 14 px.
...
| `options.height` | `number` | `260` | Минимальная высота области прокрутки. |
| `options.cardWidth` | `number` | `280` | Максимальная ширина одной карты. |
| `options.cardHeight` | `number` | `Math.floor(cardWidth * 1.05)` | Базовая высота карты для расчёта шага прокрутки. |
| `options.gap` | `number` | `3` | Отступ между картами. |
