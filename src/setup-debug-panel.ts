...
export type SetupDebugPanelOptions = {
    document: Document
    toggleButtonParent?: HTMLElement | null
...
export function setupDebugPanel({
    ...
    const toggleParent = toggleButtonParent ?? doc.body
    if (!toggleParent) {
        throw new Error('Не удалось определить контейнер для кнопки переключения debug панели')
    }

    const debugToggleButton = doc.createElement('button')
    ...
    toggleParent.appendChild(debugToggleButton)
